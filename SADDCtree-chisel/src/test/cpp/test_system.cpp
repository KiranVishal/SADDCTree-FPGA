...
class t_Saddc : public XFilesDana_api_t { //check this API
private:
  uint64_t cycle;
  bool vcd_flag;
  FILE * vcd;
...
int t_Saddc::read_parameters(const string file_string_parameters) {
...
t_Saddc::t_Saddc(const string file_string_vcd) {
  seed = time(NULL);
  srand(seed);
  xfiles_dana = new XFilesDana_t();
  xfiles_dana->init(seed);
...
void t_Saddc::tick_lo(int reset) {
  xfiles_dana->clock_lo(dat_t<1>(reset));
}

void t_Saddc::tick_hi(int reset) {
  if (vcd_flag)
...
int t_Saddc::tick(int num_cycles = 1, int reset = 0,
...
void t_Saddc::reset(int num_cycles) {
  for(int i = 0; i < num_cycles; i++) {
    tick(1,1);
  }
}

int t_Saddc::new_write_request(uint32_t nnid, std::vector<response> * outputs = NULL,
                              bool debug = false) {
...
void t_Saddc::write_data(int tid, int32_t data, int is_last,
...
void t_Saddc::write_rnd_data(int tid, int num, int decimal) {
  for (int i = 0; i < num; i++)
    write_data(tid, 256, (i == num - 1));
}

int t_Saddc::new_read_request(int tid, std::vector<response> * outputs = NULL,
                            bool debug = false) {
  uint64_t funct, rs1, rs2;
  // Compute the fields;
  funct = 0 & ~(1 << 1) & ~(1 << 2);
...
void t_Saddc::set_asid(uint16_t asid) {
...
void t_Saddc::cache_load(int index, uint32_t nnid, const char * file,
                        bool debug = false) {
  std::stringstream ss("");
...
int t_Saddc::is_done (uint16_t _asid, uint16_t _tid) {
  std::string string_table("XFilesDana.xFilesArbiter.tTable.table_");
  std::stringstream string_field("");
  for (int i = 0; i < parameters.transaction_table_num_entries; i++) {
    uint16_t asid, tid;
...
uint64_t t_Saddc::get_cycles() {
  return cycle;
}

int t_Saddc::run_single(transaction * t, bool debug = false,
...
int t_Saddc::run_smp(std::vector<transaction *> * transactions,
...
int t_Saddc::testbench_fann(const char * file_net,
...
int t_Saddc::testbench_fann(std::vector<const char *> * files_net,
...
int main(int argc, char* argv[]) {
  // t_Saddc* api = new t_Saddc("build/t_Saddc.vcd");
  t_Saddc * api;
  ...
  // Run the constructor for t_Saddc specifying a vcd file if we have
  // one
  if (has_vcd) api = new t_Saddc(file_vcd);
  else api = new t_Saddc();
